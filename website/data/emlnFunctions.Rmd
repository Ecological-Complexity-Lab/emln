...
In this Rmd, I will show how I built the functions that query the database.

First, there is a folder called "datasets", where all the networks are stored by the name format 'id_type_author_yearPublished' or if its a network comprised of several papers published at different years, the format is 'id_type_author'

Second, I will create lists that contain all of the information that is available from each network by layers, nodes and general information about the network
```{r libraries needed}
library(ggplot2)
library(readr)
library(tibble)
...
```{r Creation of lists for each csv file type}
...
#read the file path
parent.folder <- "~/GitHub/emln_package/data/data-raw/"
...
id_readout <- data.frame(id = 1:78, network_name = sub.folders.short[str_order(sub.folders.short, numeric = T)], network_path = sub.folders.long)
...
ggsave("~/GitHub/emln_package/data/data-raw/pie_multi_types.pdf",plot = p2,width =  10, height = 6)

#save ecological types
ggsave("~/GitHub/emln_package/data/data-raw/pie_eco_types.pdf",plot = p3,width =  10, height = 6)
...
ggsave("~/GitHub/emln_package/data/data-raw/net_types_array.pdf",plot = p5,width =  10, height = 6)

```

```{r Layers Information}
...
p4
ggsave("~/GitHub/emln_package/data/data-raw/layer_types.pdf",plot = p4,width =  10, height = 6)


#combination of layer type and network type
multi_eco <- left_join(multi,eco)
...
p10 <- ggplot(multi_eco, aes(x=multi_type,y=n, fill=eco_type)) + 
  ...
  theme(axis.text.x = element_text(angle = 75, vjust = 0.5, size = 12),legend.title = element_text(size=13)) + ylab('Count')  + scale_fill_nejm(name = 'Interaction Type')
p10 

ggsave("~/GitHub/emln_package/data/data-raw//multi_layer_types.pdf",plot = p10,width =  10, height = 6)
...
coord <- left_join(coord,multi[,c('network_id','multi_type')])
...
p6 <- ggplot(worldmap) +
  ...
  geom_point(
    ...
    alpha = 0.8) + expand_limits(x = worldmap$long, y = worldmap$lat) +
  theme_void() + theme(title = NULL)  + scale_color_nejm() + labs(colour="Multilayer Type") +
  theme(legend.position="bottom",legend.title = element_text(size=13))

p6

ggsave("~/GitHub/emln_package/data/data-raw/worldmap_plot_new.pdf",width = 25,height =15) 
...
grid <- ggpubr::ggarrange(
          ...
          ggpubr::ggarrange(p10, labels = "(C)", hjust=-0.1), 
          ggpubr::ggarrange(p6, labels = "(D)"), 
          nrow = 3,legend = "right")
grid

ggsave('~/GitHub/emln_package/data/data-raw/emln_metadata_figure.pdf',grid,width = 10, height = 18)
```
